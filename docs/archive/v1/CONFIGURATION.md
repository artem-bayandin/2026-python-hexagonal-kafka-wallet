...
| `production` | Controlled deployment only. | Demo OTP output and static admin access are disabled and cannot be enabled. |
...
The frontend must not receive `JWT_SECRET`, `OTP_HMAC_SECRET`, `ADMIN_API_KEY`, or database URLs. `VITE_*` values are public build-time values.
...
- Redact all secret values, JWTs, OTPs, admin keys, and database connection strings from logs, traces, error responses, and diagnostics records.

...
class OtpChallengeModel(Base):
    ...
    __table_args__ = (
        ...
        # The app increments it on failed verification and compares it to a max
        # (e.g. OTP_MAX_ATTEMPTS) to lock the challenge.
        ...
        # Why it exists: Most OTP queries are scoped to one user — e.g. “get the
        # current challenge for this user” or “list this user’s challenges, newest first.”
        # Indexing user_id first narrows to that user; created_at DESC matches
        # “most recent first” without a separate sort.
        # This speeds up reads like get_current_for_user_for_update.
        # Note: text("created_at DESC") tells PostgreSQL to store the index in
        # descending order on created_at.
        Index(
            "ix_otp_challenges_user_id_created_at",
            "user_id",
            text("created_at DESC"),
        ),
        # What it does: A partial unique index on user_id, but only for rows where
        # both consumed_at and invalidated_at are NULL.
        ...
        # - After successful verification → consumed_at is set → row leaves the index
        #   → uniqueness no longer applies to it.
        ...
        # this prevents two concurrent requests from both leaving a “current”

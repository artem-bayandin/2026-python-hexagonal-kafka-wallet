...
```sh
docker compose up -d postgres kafka worker
```
...
1. Generate it with `uv run alembic revision --autogenerate -m "<summary>"`.
...
3. Pass frontend lint, typecheck, tests, and production build.
4. Pass outbox, idempotency, ordering, and recovery tests for version 2.
5. Apply migrations before application rollout when they are backward compatible.
6. Verify readiness, error rate, migration state, outbox age, and worker lag before declaring success.
...
- **Database unavailable:** fail readiness, stop accepting mutations, preserve error evidence without connection strings, and recover from the last verified backup only under an approved incident procedure.
- **Kafka unavailable:** continue accepting version-2 requests only while the transactional outbox is durable and its age/backlog alert threshold is not exceeded; retry relay publication with backoff.
- **Worker failure:** restart the worker; rely on inbox/outbox state and idempotent handlers rather than manual balance replay.
- **Secret exposure:** rotate the affected secret, revoke affected sessions, remove the leaked material from logs/diagnostics, and investigate access.

Diagnostics data is a development-only aid, not an incident datastore or a replacement for structured logs, metrics, traces, backups, or audit controls.

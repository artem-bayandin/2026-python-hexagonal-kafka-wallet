...
- **Tables** (migration `d377d8c90992`): `currencies`, `user_wallets`, `admin_wallets`, `transactions`; USD/USDT and admin wallets seeded.

### Phase 4 — Admin wallet

- Reference routes (`GET /reference/currencies`, `GET /reference/users`), admin deposit, admin balances, admin transactions, development Admin UI.

### Phase 5 — User wallet

- User balances, transactions, exchange, withdrawal, transfer; Wallet UI with embedded history.
...
- **Domain:** `Result[T]`, OTP/auth read models (`OtpChallengeItem`, `AuthSessionItem`, `UserItem`), ports, use-case handlers.
- **Adapters:** `app/auth/` (HMAC OTP, PyJWT, system clock), `app/db/` (models, mappers, CQRS repositories).
- **API:** routers, executors, Pydantic schemas, `unwrap_domain_result` / `DomainResultError`, `ContextVarCurrentUserProvider`.
- **UI:** login flow, demo OTP display, token in `sessionStorage`, authenticated health check, logout, Wallet page.

### Not yet done

- Kafka / Version 2.
- Automated tests (`backend/tests/` directories exist; no test modules yet).
...
- **Façade imports:** cross-layer imports use package `__init__.py` façades only (`from app.domain import Result`, not `app.domain.read_models.user`). Same-layer imports use relative paths through subpackage façades.
- **Read models:** persisted/query shapes live in `domain/read_models/` (`UserItem`, `OtpChallengeItem`, `BalanceItem`, …). There is no separate `domain/entities/` package in the current codebase.
...
- **Error mapping:** only in `backend/app/api/exception_handlers.py`. Routes use `unwrap_domain_result()` after executors return.
- **Executors:** one file per use case under `backend/app/api/executors/`; query routes use `read_session`, commands use `write_session` ([db_session.py](../../backend/app/api/db_session.py)).
...
| Phase 3 | Step 5 (ORM models + migration subset) | Done |
| Phase 4 | Steps 2, 4, 5 (repos), 8 (admin), 9 (admin routes), 10 (admin UI) | Done |
| Phase 5 | Step 8 (user), 9 (user routes), 10 (wallet UI) | Done |
...
- Next phase (Kafka): [PHASE_6_KAFKA.md](PHASE_6_KAFKA.md)

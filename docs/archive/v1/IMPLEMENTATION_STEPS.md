...
Work from top to bottom. Each step states what to build and what proves it is complete.
...
**Done when:** domain unit tests run with no imports from FastAPI, Pydantic, SQLAlchemy, or PyJWT, and the command/query dependency direction can be explained from the code.
...
- [ ] Inject `CurrentUserProvider` into authenticated HTTP wallet handlers instead of repeating `current_user` fields in their command/query DTOs; keep admin identity explicit to its own inputs.
...
- [x] Treat provider access without a bound user as an unexpected wiring error, and do not use the HTTP current-user provider in admin execution paths.
...
**Done when:** version 1 is reproducible from a fresh clone and all checks pass. Tag or otherwise mark this point before starting the next version.

## Required completion controls

- [ ] Add pre-commit checks for ruff, mypy, and fast unit tests.
- [ ] Add CI for backend lint/typecheck/tests, frontend typecheck/tests/build, and PostgreSQL integration tests.
- [ ] Add dependency vulnerability scanning and a recurring dependency-review process.
- [ ] Add architecture decision records for CQRS, generic `Result[T]` plus automatic transaction contexts, PostgreSQL, and OTP sessions.

## Optional final phase — HTTP request idempotency

Do not implement this phase unless it is explicitly requested. It is intentionally last and is not required for the baseline application version.

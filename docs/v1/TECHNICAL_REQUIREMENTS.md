...
The product behavior is defined in [FUNCTIONAL_REQUIREMENTS.md](FUNCTIONAL_REQUIREMENTS.md). This document defines the technical architecture and constraints for implementing version 1. [README.md](README.md) defines document authority and reading order.
...
- Configuration, profile boundaries, local ports, and polling defaults are defined in [CONFIGURATION.md](CONFIGURATION.md).

### 2.4 Supported-version and dependency policy
...
- The current baseline is Python 3.14.6, FastAPI 0.139.2, Pydantic 2.13.4, `pydantic-settings` 2.14.2, SQLAlchemy 2.0.51, `uv` 0.11.31, `testcontainers` 4.14.2, and `email-validator` 2.3.0.
...
- incoming adapters translate HTTP messages into commands/queries;
- outgoing ports describe persistence, token, OTP, and clock needs;
- PostgreSQL, FastAPI, and PyJWT are outer adapters;
...
`domain/` must not import FastAPI, Pydantic, SQLAlchemy, or PyJWT packages. Dependencies point inward:

```
api ──> domain commands and queries <── ports
db / auth adapters ──────────────────> ports
```
...
User transaction history is scoped by wallet ownership (`source_wallet_id` or `dest_wallet_id` in the user's wallet IDs). Phase 5 query repositories use a CTE + `IN` pattern. Admin transaction listing queries all rows.
...
Unexpected infrastructure and programming failures are not converted to `Result.failure`. They propagate as exceptions so the active transaction rolls back and the API can apply its unexpected-failure policy.
...
Use one async SQLAlchemy session per HTTP command:
...
The session dependency or command executor supplies the transaction boundary. Domain handlers depend directly on repository Protocols and do not receive SQLAlchemy sessions or a custom Unit of Work. Every repository participating in one command shares the same session.
...
- The OTP is returned only when `APP_ENV=development` and `ENABLE_DEMO_OTP=true`; it must not appear in logs.
...
Only authenticated HTTP use cases use this provider. OTP request/verification, development admin operations, and other non-request execution paths receive identity explicitly from their own inputs when needed. Do not let detached/background tasks depend on the request provider because Python task creation can copy `ContextVar` state; pass any required identity explicitly instead.
...
Success status codes remain route-specific (`200`, `201`, `204`) and are not selected by `unwrap_domain_result`. Request-validation failures bypass `Result[T]` and remain `422 VALIDATION_ERROR`; uncaught exceptions remain `500 INTERNAL_ERROR`.

Version 1 wallet commands return completed results. HTTP idempotency keys are deferred to the roadmap's final optional hardening phase.
...
The frontend contains Login, Wallet (including paginated history), and Admin views. Version 1 switches views with React state in `App.tsx`; URL routing is optional.
...
- Admin views are development-only; production builds must omit those views until production authorization and observability controls exist.
- Error responses and failed statuses are visible in simple text UI.
...
- No FastAPI, SQLAlchemy, or PostgreSQL is needed.
...
- rendering completed and failed states.
...
- Pre-commit and CI are optional. They run lint, formatting, type checking, unit tests, integration tests, and the frontend test/build.
